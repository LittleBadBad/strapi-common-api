...
import qs from "qs";
import {BaseType, MergeAttrs, Query, RelationData, ResponseMeta, UserInfo} from "./types";
 ...
 */
export function getMany<T extends BaseType = BaseType>(type: string, query?: Query<T>): Promise<Return<T[]>> {
 ...
 * @param query strapi query object
 */
export function getOne<T extends BaseType = BaseType>(type: string, id: number, query?: Query<T>): Promise<Return<T>> {
 ...
 * @param query strapi query object
 */
export function post<T extends BaseType = BaseType>(type: string, data: InputData<T>, query?: Query<T>): Promise<Return<T>> {
 ...
 * @param query strapi query object
 */
export function put<T extends BaseType = BaseType>(type: string, id: number, data: InputData<T>, query?: Query<T>): Promise<Return<T>> {
 ...
 * @param query strapi query object
 */
export function remove<T extends BaseType = BaseType>(type: string, id: number, query?: Query<T>): Promise<Return<T>> {
    return strapiRequest.delete(`/${type}/${id}?${qs.stringify(query, {encodeValuesOnly: true})}`)
        .then(r => r.data)
}

export interface UserLogged<T = UserInfo> {
    user: T & { id: number },
    jwt: string
}

export function login<T = UserInfo>({identifier, password}: {
    identifier: string,
    password: string,
...
export function register<T = UserInfo>({username, email, password}: {
    username: string
    email: string
    password: string
}): Promise<UserLogged<T>> {
...
export function emailConfirm(token: string) {
